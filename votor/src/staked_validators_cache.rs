...
impl StakedValidatorsCache {
    ...
    /// Publish the current epoch's staked-pubkey set (union'd with
    ...
    fn maybe_refresh_admission(&mut self, epoch: Epoch) {
        ...
        let mut set = datagram_endpoint::current_admit_set(&self.bank_forks);
        set.extend(self.extra_admit.iter().copied());
        admission.swap(set);
